...
def train(checkpoint_path=None, use_strict_resume=False):
    ...
    PAD_TOKEN = 2
    BIN_OFFSET = NUM_SPECIAL_TOKENS + NUM_MECH_TYPES
    ...
    if rank == 0:
        wandb.init(
            ...
            name=f"coords_ce_mse_clip_d{model_config['d_model']}_h{model_config['h']}_n{model_config['N']}",
            config=model_config,
        )
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader), desc=f"[Rank {rank}] Val {epoch}", disable=rank != 0) as pbar:
                for batch in val_loader:
                    ...
                    pbar.set_postfix({"ValLoss": total_loss.item(), "ValAcc": batch_acc.item()})
                    pbar.update(1)
