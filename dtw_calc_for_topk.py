...
# USER SETTINGS
# =========================================================
TEMPERATURE = 0.25     # <-- Change this and rerun
TOP_K = 50            # <-- Set None to disable top-k

# Always test these SAME sample indices
FIXED_SAMPLE_IDXS = [
...
# =========================================================
# AUTOREGRESSIVE PREDICTION WITH TOP-K + PROBABILITY DEBUG
# =========================================================
DEBUG_PROBS = True   # <-- Set False to hide probability prints

def predict_autoregressive_latent(model, latent, mech_idx, max_len, temperature, top_k=None):
    ...
    with torch.no_grad():
        for step in range(max_len):
            ...
            # ---- DEBUG PRINTING ----
            if DEBUG_PROBS:
                # Compute entropy to see distribution sharpness
            ...
            if top_k is not None and top_k > 0:
                ...
                # Select top-k from full distribution
                ...
                if DEBUG_PROBS:
                    print(f"\nTop-{top_k} tokens:", topk_idx[0].tolist())
                    print(f"Top-{top_k} probs :", [float(p) for p in topk_probs[0]])
                ...
                # Normal sampling (full distribution)
                if temperature == 0:
...
def process_curve_with_index(idx):
    ...
    # Clean GT labels
    gt_raw = labels_cont[idx]
    gt_points = clean_and_reshape_label(gt_raw)

    if gt_points.shape[0] == 0:
        print("Invalid GT, skipping.")
        return
    ...
    if Pp is None or Pp.shape[0] < minsteps:
        print("Prediction simulation failed.")
        return

    px, py = Pp[:, cidx, 0], Pp[:, cidx, 1]
...
def main():
    ...
    if GLOBAL_DTW_VALUES:
        ...
        avg = np.mean([d for (_, d) in GLOBAL_DTW_VALUES])
        print("----------------------------------------")
        print(f"Average DTW: {avg:.6f}")
