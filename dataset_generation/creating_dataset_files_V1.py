...
# ------------------------------------------------------------------
# Extended Inspection (includes VAE μ and σ)
# ------------------------------------------------------------------
def inspect_dataset_with_latents(dataset, vae_checkpoint, latent_dim=50, num_samples=5):
    from vae import VAE
    ...
    vae.eval()
    print("\n✅ Loaded VAE model for latent inspection")
    print("=" * 100)
    print("FULL DATASET + LATENT INSPECTION")
    print("=" * 100)
    ...
    for i in range(min(num_samples, len(dataset))):
        ...
        sigma_np = sigma.cpu().numpy().squeeze()
        ...
        print("\n--- VAE Latents ---")
        print(f"μ (mean): shape={mu_np.shape}, mean={mu_np.mean():.3f}, std={mu_np.std():.3f}")
        print(f"σ (std):  shape={sigma_np.shape}, mean={sigma_np.mean():.3f}, std={sigma_np.std():.3f}")
        print(f"μ first 10 dims: {np.round(mu_np[:10], 3)}")
        print(f"σ first 10 dims: {np.round(sigma_np[:10], 3)}")

        print("-" * 100)

    print("\n✅ Inspection complete — verify above samples before generating dataset.")

# ------------------------------------------------------------------
# NPY File Creation (with VAE μ and σ)
# ------------------------------------------------------------------
def create_npy_files(dataset, output_dir, vae_checkpoint, latent_dim=50):
    from vae import VAE  # import here to avoid circular deps
    ...
    vae.load_state_dict(checkpoint["state_dict"])
    vae.eval()
    print("✅ Loaded VAE model for latent extraction")
    ...
    for i in tqdm(range(len(dataset))):
        ...
        with torch.no_grad():
            ...
            sigma = torch.exp(0.5 * logvar)
        all_mu.append(mean.cpu().numpy().squeeze(0))
    ...
    np.save(os.path.join(output_dir, "vae_mu.npy"), all_mu)
    # np.save(os.path.join(output_dir, "vae_sigma.npy"), all_sigma)
    ...
    print(f"Saved latent files: vae_mu.npy and vae_sigma.npy")
...
if __name__ == "__main__":
    ...
    if response.lower() == 'y':
        create_npy_files(dataset, output_dir, vae_checkpoint, latent_dim=50)
